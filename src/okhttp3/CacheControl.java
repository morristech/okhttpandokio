...
public final class CacheControl {
	 ...
	 */
	// 强制走网络
	public static final CacheControl FORCE_NETWORK = new Builder().noCache()
			.build();
	 ...
	 * requires server validation, the call will fail with a
	...
	private final boolean noCache;// 不使用缓存，全部走网络
	private final boolean noStore;// 不使用缓存，也不存储缓存
	...
	private final boolean onlyIfCached;// 只使用缓存
	private final boolean noTransform;

	String headerValue; // Lazily computed, null if absent.

	private CacheControl(boolean noCache, boolean noStore, int maxAgeSeconds,
			int sMaxAgeSeconds, boolean isPrivate, boolean isPublic,
			boolean mustRevalidate, int maxStaleSeconds, int minFreshSeconds,
	...
	/**
	 * This field's name "only-if-cached" is misleading. It actually means
	 * "do not use the network". It is set by a client who only wants to make a
	...
	public static CacheControl parse(Headers headers) {
		...
		for (int i = 0, size = headers.size(); i < size; i++) {
			...
			if (name.equalsIgnoreCase("Cache-Control")) {
				if (headerValue != null) {
					// Multiple cache-control headers means we can't use the raw
					// value.
			...
			while (pos < value.length()) {
				...
				if ("no-cache".equalsIgnoreCase(directive)) {
					...
					maxStaleSeconds = HttpHeaders.parseSeconds(parameter,
							Integer.MAX_VALUE);
		...
		return new CacheControl(noCache, noStore, maxAgeSeconds,
				sMaxAgeSeconds, isPrivate, isPublic, mustRevalidate,
				maxStaleSeconds, minFreshSeconds, onlyIfCached, noTransform,
				headerValue);
	}
	...
	public static final class Builder {
		...
		/**
		 * Sets the maximum age of a cached response. If the cache response's
		 * age exceeds {@code maxAge}, it will not be used and a network request
		 * will be made.
		 *
		 * @param maxAge
		 *            a non-negative integer. This is stored and transmitted
		 *            with {@link TimeUnit#SECONDS} precision; finer precision
		 *            will be lost.
		 */
		...
		/**
		 * Accept cached responses that have exceeded their freshness lifetime
		 * by up to {@code maxStale}. If unspecified, stale cache responses will
		 * not be used.
		 *
		 * @param maxStale
		 *            a non-negative integer. This is stored and transmitted
		...
		/**
		 * Sets the minimum number of seconds that a response will continue to
		 * be fresh for. If the response will be stale when {@code minFresh}
		 * have elapsed, the cached response will not be used and a network
