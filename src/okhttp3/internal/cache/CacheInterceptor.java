...
public final class CacheInterceptor implements Interceptor {
	...
	public Response intercept(Chain chain) throws IOException {
		Log.i(CldMainUtil.TAG, "CacheInterceptor");
		...
		if (cacheCandidate != null && cacheResponse == null) {// 缓存不可用，直接关闭
		...
		// insufficient, fail.
		if (networkRequest == null && cacheResponse == null) {// 既不能使用网络，也不能使用缓存，那么返回失败的请求
			return new Response.Builder().request(chain.request())
					.protocol(Protocol.HTTP_1_1).code(504)
					.message("Unsatisfiable Request (only-if-cached)")
					.body(EMPTY_BODY).sentRequestAtMillis(-1L)
					.receivedResponseAtMillis(System.currentTimeMillis())
					.build();
		}

		// If we don't need the network, we're done.
		if (networkRequest == null) {// 如果不使用网络，直接使用缓存
		...
		if (cacheResponse != null) {
			if (validate(cacheResponse, networkResponse)) {// true的话应该使用缓存，否则使用网络数据
				Log.i(CldMainUtil.TAG + "cache", "validate true");
				...
				Log.i(CldMainUtil.TAG, "cache" + "validate false");
		...
		if (HttpHeaders.hasBody(response)) {
			Log.i(CldMainUtil.TAG, "cache" + "hasBody");
	...
	private CacheRequest maybeCache(Response userResponse,
			Request networkRequest, InternalCache responseCache)
			throws IOException {
