...
public final class RealInterceptorChain implements Interceptor.Chain {
	...
	public Response proceed(Request request, StreamAllocation streamAllocation,
			HttpStream httpStream, Connection connection) throws IOException {
		if (index >= interceptors.size())
			throw new AssertionError();// 不能超过最大size。

		calls++;

		// If we already have a stream, confirm that the incoming request will
		// use it.
		...
		RealInterceptorChain next = new RealInterceptorChain(interceptors,
				streamAllocation, httpStream, connection, index + 1, request);
		...
		// chain.proceed().
		if (httpStream != null && index + 1 < interceptors.size()
				&& next.calls != 1) {
			throw new IllegalStateException("network interceptor "
